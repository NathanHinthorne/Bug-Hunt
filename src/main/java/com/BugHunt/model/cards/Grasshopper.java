...
public class Grasshopper extends Card {
    ...
    // * makes sense given the fact we're ALREADY receiving the player's collection
    // as a parameter

    // (current approach)
    // 2. we call getPoints() on every card when we need to update the points (i.e.
    // every time we add a card to the collection)
    // * less efficient
    // * more intuitive
